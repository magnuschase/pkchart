Feature: Managing personal card portfolio
  ...
  Background:
    Given The card "Latias EX" from set "Paradise Dragona" in condition "CGC 9.5" exists with a current price of 600.00 PLN
  ...
  Scenario: User views portfolio statistics
    Given The user's portfolio contains "Latias EX" from set "Paradise Dragona" and "Pikachu with Grey Felt Hat" from set "SVP Black Star Promos"
